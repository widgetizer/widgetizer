  ...
  Object.entries(TETROMINOES).forEach(([type, tet]) => {
    ...
    for (let rotation = 0; rotation < tet.blocks.length; rotation++) {
      ...
      for (let blockIndex = 0; blockIndex < 4; blockIndex++) {
        ...
        const radii = getCornerRadiiConfig(shape, currentBlockLocalPos, CORNER_RADIUS);
...
function canPlace(tetromino, x, y, rotation, otherTetromino) {
  ...
  function rotateTetromino(tetromino) {
    ...
    if (canRotate) {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        const blockWorldX = (tetromino.x + blockLocalPos[0]) * BLOCK_SIZE + BLOCK_SIZE / 2;
        const blockWorldY = (tetromino.y + blockLocalPos[1]) * BLOCK_SIZE + BLOCK_SIZE / 2;
      
        // Use pre-generated texture based on side, type, rotation, and block index
        const textureKey = getBlockTextureKey(tetromino.side, tetromino.type, tetromino.rotation, i);
        const sprite = tetromino.scene.add.sprite(blockWorldX, blockWorldY, textureKey);
        sprite.setDepth(1);
  ...
  function spawnTetromino(scene, side) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (!scene.textures.exists(textureKey)) {
        console.error(`Error: Texture ${textureKey} not found! This should never happen with pre-generated textures.`);
      }
  ...
  function spawnGhostTetromino(scene, side, activeTetromino) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (!scene.textures.exists(textureKey)) {
        console.error(`Error: Ghost texture ${textureKey} not found! This should never happen with pre-generated textures.`);
      }
